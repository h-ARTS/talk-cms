...
describeWithMongo("MongoDbPageRepository", () => {
  test("inserts and reads a page from the pages collection", async () => {
    ...
    try {
      ...
      await expect(repository.update(page.id, { blocks: [], name: null })).resolves.toBe(true)
      await expect(repository.findById(page.id)).resolves.toMatchObject({ blocks: [], name: null })
      await expect(collection.findOne({ _id: page.id })).resolves.toMatchObject({
        _id: page.id,
        blocks: [],
        createdAt: page.createdAt,
